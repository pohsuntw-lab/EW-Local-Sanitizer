...
for (const [path, metadata] of Object.entries(lock.packages ?? {})) {
  ...
  if (metadata.hasInstallScript === true || metadata.gypfile === true) violations.push(`${name} introduces install-script or native-build risk`);
  if (!dependencyNotes.includes(`\`${name}\``)) violations.push(`${name} is missing from SECURITY_DEPENDENCIES.md`);
}
...
console.log("Dependency policy check passed: exact lock, integrity, licenses and no install/native build flags.");
