...
| C1–C7 deterministic detection, random IDs and encrypted dictionary | `detection-transform.test.ts` current GitHub token shapes, private-key armor, quoted/unquoted credential, overlap and identity tests plus positive/negative fixtures | Pass for implemented plain-text detectors |
