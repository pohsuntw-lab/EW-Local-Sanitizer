...
test("writes allowlisted package, validates actual ZIP SHA-256 and excludes local artifacts", () => {
  ...
  const findings = detectText(source.text, detection);
  const registry = ProjectTokenRegistry.create(detection.dictionary.projectId, detection.dictionary);
  const staleTokenMap = encryptTokenMap(registry, "correct horse battery staple");
  ...
  const missingTokenMap = verifyForExport(verificationRequest(source, transformed, detection));
  ...
  assert.equal(verifyForExport(verificationRequest(source, transformed, detection, staleTokenMap)).status, "blocked");
  const forgedTokenMap = { payload: staleTokenMap.payload, proof: Object.freeze({}) } as typeof staleTokenMap;
  assert.equal(verifyForExport(verificationRequest(source, transformed, detection, forgedTokenMap)).status, "blocked");
  const tokenMap = encryptTokenMap(registry, "correct horse battery staple");
  const damagedTokenMap = { payload: Buffer.from(tokenMap.payload), proof: tokenMap.proof } as typeof tokenMap;
  damagedTokenMap.payload[0] = (damagedTokenMap.payload[0] ?? 0) ^ 0x01;
  assert.equal(verifyForExport(verificationRequest(source, transformed, detection, damagedTokenMap)).status, "blocked");
  const outcome = verifyForExport(verificationRequest(source, transformed, detection, tokenMap));
  assert.equal(outcome.status, "verified");
  ...
  assert.equal(JSON.parse(manifestEntry.data.toString("utf8")).token_map_created, true);
