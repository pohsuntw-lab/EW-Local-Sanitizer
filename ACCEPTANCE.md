...
- G11: The assisted NSIS installer displays the versioned Traditional Chinese or English EW Local Sanitizer EULA before installation, leaves its acceptance checkbox unselected by default, and requires explicit acceptance to continue. The human-maintained EULA source is projected into NSIS text at build time; a local manifest binds each locale to its SHA-256. Upgrade installs display the same acceptance page. Silent installation fails closed unless `/EWLSACCEPTEULA=<agreement-version>:<current-locale-sha256>` matches, and successful installation records only the accepted version, locale, hash and installation metadata. The agreement preserves third-party license applicability and does not claim perfect detection, anonymity or compliance.
